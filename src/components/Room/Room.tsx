...
const SocketClient: React.FC<SocketClientProps> = ({ roomCode }) => {
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (typeof message === "object") {
                ...
                if (message.event === "userseek") {
                    console.log(message);
                    ws?.send(JSON.stringify({ command: "player_seek", additional: `${message.data}` }));
                }
            }
        };
    ...
    return (
        <div style={{ top: "50%", position: "absolute", color: "#a94444 !important" }}>
            <h1>Room: {roomCode}</h1>
            <iframe id="myIframe" src="https://moonanime.art/iframe/vxguiwozjg/?i=12" width="600" height="400"></iframe>
        </div>
    );
};
